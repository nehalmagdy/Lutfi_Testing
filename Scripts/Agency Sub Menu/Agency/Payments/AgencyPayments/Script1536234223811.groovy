	...
	if(np_rows_count==cnPay){
		while(npayRes.next()){
			...
			for (int row = 0; row < np_rows_count; row++) {
				...
				String DBDate = npayRes.getString(2).toString() // date rage3 b .0 fel a5er ?!!
			...
			if(!Found){
				//Fail: a paid trasaction doesn't appear in the list
				AllFound = false;
	...
	if(p_rows_count==cPay){
		while(payRes.next()){
			...
			for (int row = 0; row < p_rows_count; row++) {
				...
				String DBDate = payRes.getString(2).toString().split(".")[0]
