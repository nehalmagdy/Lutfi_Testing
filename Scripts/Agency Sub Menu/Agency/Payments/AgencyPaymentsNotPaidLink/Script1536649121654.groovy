...
if (rows_count > 0) {
	...
	if(np_rows_count>0){
		//test on the first row
			...
			// check if the row is transfered to the paid table
			// get the paid table rows
			driver = DriverFactory.getWebDriver()
			...
			boolean found = false;
			for (int row = 0; row < p_rows_count; row++) {
				...
				if(cellAmount.equals(pcellAmount) && pcellDate.equals(cellDate) && cellStatus == "Paid"){
					found=true;
					break;
				}
			}
			if(!found){
				...
				log.logFailed("Failed: The Entry wasnot moved to the paid table")
			}
			else{
				log.logPassed("Passed: The Entry is moved successfully to the paid table")
			}
