...
import com.kms.katalon.core.logging.KeywordLogger as KeywordLogger
	...
	if (rows_count > 0) {
		...
		String name = Columns_row[1].getText()
		
		String mobQuery = "select mobile from agency where id = "+id;
		ResultSet mobRes = CustomKeywords.'com.database.lutfi.Database.executeQuery'(mobQuery)
		mobRes.next()
		String mob = mobRes.getString(1)
		...
		String aname = WebUI.getText(findTestObject('AgencyControlPage/TransactionsPage/lbl_AgencyName')).split('\n')[0].trim()
		String mobile = WebUI.getText(findTestObject('AgencyControlPage/TransactionsPage/lbl_AgencyMobile'))
		
		if(mobile.equalsIgnoreCase(mob) && name.equalsIgnoreCase(aname)){
			log.logPassed("Correct Agency name and Mobile")
		}
		else{
			log.logFailed("InCorrect Agency name or Mobile")
		}
		...
		// to get only the number between ()
		...
		if((bAmount == Integer.parseInt(agencyBalance)) && (pAmount == Integer.parseInt(payAmount)) && (gAmount == Integer.parseInt(getAmount))){
			trueAmounts=true;
		...
		////////////////////////////////////////////////////////////////////////////////////////
			...
			if(AllFound){
				...
				log.logPassed("Pass: Get the Shown Data is complete")
			}
			else{
				//Fail: the result is incorrect for the search query
				log.logFailed("Fail: Get the Shown Data is Incomplete")
		...
		/////////////////////////////////////////////////////////////////////////////
			...
			if(AllFound){
				...
				//Fail: the result is incorrect for the search query
				log.logFailed("Fail: PAY the Shown Data is Incomplete")
